...
if(isset($school_id))
{
 ...
 $disable = ($user_type == 1 or $user_type== 5)? "" :"disabled";
    ...
    //CHECK CLIENT OS
    //USING CLIENT OS FILE SYSTEM
    //CHECK IF THE CLIENT HAS A UNIQUE ACCESS KEY FOR THE CURRENT DEVICE - OPEN AND SEND TO GET USER ACCESS DETAILS
    //ELSE GENERATE KEY UPON REGISTRATION
    ...
    $key_found = TRUE;
    $keyfile = fopen("$device_key_path", "r+") or $key_found = FALSE;
        
    $client_key = fgets($keyfile);

    if ($client_ip = '127.0.0.1'){
        $q = "SELECT * FROM schoollms_schema_securitydata_offline_access_register";
    } else {
        $q = "SELECT * FROM schoollms_schema_securitydata_online_access_register WHERE client_device_key LIKE '%$client_key%'";
    }
    $data->execSQL($q);
    ...
    if ($client_ip = '127.0.0.1'){
        ...
        $q = "SELECT * FROM schoollms_schema_securitydata_online_access_register WHERE client_ip = '$client_ip'";
    }
    $data->execSQL($q);
    ...
    if ($found && $client_ip == '127.0.0.1'){
        alert("You are not registered as the user of this device - Please click below to register");
        redirect("http://$localhost/vas/timetable/register.php");
        //"<a href=''> REGISTER DEVICE AS OWN </a>";
    } elseif($access_status == 'inactive') {
        alert("Your ACCESS is INACTIVE - Please contact School System Admin to ACTIVATE your ACCESS");
        exit;
    } else {
        alert("YOUR TIMETABLE IS NOT SETUP FOR OFFLINE ACCESS WHICH IS REQUIRED TO GO ONLINE\nPLEASE CONTACT SCHOOL SYSTEM ADMIN FOR MORE HELP");
        exit;
    }
}
